...
import { Edit3, Trash2, Share2, Calendar, Clock, CheckCircle2, User, Link, ExternalLink, Tag } from "lucide-react";

const TaskList = ({ tasks, setEditingTask, onDelete, fetchTasks, isSharedView = false, activeTag }) => {
      ...
      {filteredTasks.length === 0 ? (
        <div className="text-center py-12 bg-gray-50 dark:bg-gray-800/50 rounded-3xl border-2 border-dashed border-gray-200 dark:border-gray-700">
          <p className="text-gray-500 dark:text-gray-400 font-medium italic">
            {activeTag ? `No tasks with tag "${activeTag}"` : isSharedView ? "No tasks shared with you yet." : "No tasks found. Time to create one! ✨"}
          </p>
        </div>
      ) : (
                ...
                <div className="flex-1 space-y-2">
                  ...
                  {task.tags?.length > 0 && (
                    <div className="flex flex-wrap gap-1.5 pt-1">
                      {task.tags.map((tag) => (
                        <span key={tag} className={`flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[11px] font-bold ${getTagColor(tag)}`}>
                          <Tag size={9} />{tag}
                        </span>
                      ))}
                    </div>
